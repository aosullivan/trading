"""Deterministic v1.7 final breakout-family prototype surface."""
...
_SPEC_PATH = _FIXTURES / "focus_basket_final_breakout_prototypes.json"
_CANONICAL_SPEC_PATH = _FIXTURES / "focus_basket_benchmarks.json"


@pytest.fixture
def final_breakout_spec():
...
def _evaluate_candidate(
    ...
    results = []
    params_by_ticker = candidate.get("params_by_ticker", {})
    for ticker in focus_chart_spec["tickers"]:
        strategy_key = candidate["ticker_strategy_map"][ticker]
        query = _chart_query(focus_chart_spec["chart_request"], ticker)
        for key, value in params_by_ticker.get(ticker, {}).items():
            query += f"&{key}={value}"
        ...
        payload = resp.get_json()["strategies"][strategy_key]
...
def test_final_breakout_candidates_match_frozen_outcomes(
    app,
    client,
    focus_chart_spec,
    focus_ohlcv_full,
    ...
    candidate = final_breakout_spec["candidates"][candidate_index]
...
def test_coin_goog_keltner_coin_esc50_is_the_best_final_breakout_candidate(
    app,
    client,
    focus_chart_spec,
    focus_ohlcv_full,
    ...
    by_key = {}
    for candidate in final_breakout_spec["candidates"]:
    ...
    assert final_breakout_spec["recommended_lead"] == "coin_goog_keltner_coin_esc50_v1"
